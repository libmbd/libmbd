...
- libMBD built with gfortran 16.2 did not export the `cmbd_nonint_density` and `cmbd_int_density` C API functions, which made the pyMBD C extension unimportable. The two are now declared public like the rest of the C API, and `mbd.h` declares them as `void`, matching their Fortran definition. The import error also now reports the underlying loader message instead of hiding it ([#150](https://github.com/libmbd/libmbd/pull/150))
