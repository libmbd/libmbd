...
def gcov_file_coverage(path):
    """Return {lineno: hit_bool} for one .gcov file, merged per source line."""
    lines = {}
    src = None
    with open(path, encoding="utf-8", errors="replace") as f:
        for row in f:
            ...
            # covered-if-any: once hit, stays hit
            lines[ln] = lines.get(ln, False) or hit
    return src, lines


def repo_relative(src):
    return src.split("/src/", 1)[1] and "src/" + src.split("/src/", 1)[1] \
        if "/src/" in src else os.path.basename(src)
...
def main():
    ...
    for g in glob.glob(os.path.join(args.gcov_dir, "*.gcov")):
        ...
        h = sum(1 for v in lines.values() if v)
        m = sum(1 for v in lines.values() if not v)
        files[name] = (h, m)  # one .gcov per source with gcov -p

    if os.path.exists(args.xml):
        for fn, (h, m) in python_coverage(args.xml).items():
            files[fn] = (h, m)
    ...
    for name in sorted(files):
        ...
        th += h
        tm += m
        cov = 100 * h / tot if tot else 0.0
        print(f"{name.replace('src/', ''):32} {cov:7.2f} {h:5d} {tot:6d}")
    total = th + tm
    print(f"{'TOTAL':32} {100 * th / total if total else 0:7.2f} {th:5d} {total:6d}")
