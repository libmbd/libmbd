...
real(dp), parameter :: ts_vdw_params(3, 102) = reshape([ &
    4.5d0, 6.5d0, 3.1d0, &  ! H
    ...
    213.7d0, 3884.5d0, 3.14d0, &  ! La
    204.7d0, 3708.33d0, 3.26d0, &  ! Ce
    215.8d0, 3911.84d0, 3.28d0, &  ! Pr
    208.4d0, 3908.75d0, 3.3d0, &  ! Nd
    200.2d0, 3847.68d0, 3.27d0, &  ! Pm
    192.1d0, 3708.69d0, 3.32d0, &  ! Sm
    184.2d0, 3511.71d0, 3.4d0, &  ! Eu
    158.3d0, 2781.53d0, 3.62d0, &  ! Gd
    169.5d0, 3124.41d0, 3.42d0, &  ! Tb
    164.64d0, 2984.29d0, 3.26d0, &  ! Dy
    156.3d0, 2839.95d0, 3.24d0, &  ! Ho
    150.2d0, 2724.12d0, 3.3d0, &  ! Er
    144.3d0, 2576.78d0, 3.26d0, &  ! Tm
    138.9d0, 2387.53d0, 3.22d0, &  ! Yb
    137.2d0, 2371.8d0, 3.2d0, &  ! Lu
    ...
    317.8d0, 4224.44d0, 3.9d0, &  ! Fr
    246.2d0, 4851.32d0, 4.98d0, &  ! Ra
    203.3d0, 3604.41d0, 2.75d0, &  ! Ac
    217.0d0, 4047.54d0, 2.85d0, &  ! Th
    154.4d0, 2367.42d0, 2.71d0, &  ! Pa
    127.8d0, 1877.1d0, 3.0d0, &  ! U
    150.5d0, 2507.88d0, 3.28d0, &  ! Np
    132.2d0, 2117.27d0, 3.45d0, &  ! Pu
    131.2d0, 2110.98d0, 3.51d0, &  ! Am
    143.6d0, 2403.22d0, 3.47d0, &  ! Cm
    125.3d0, 1985.82d0, 3.56d0, &  ! Bk
    121.5d0, 1891.92d0, 3.55d0, &  ! Cf
    117.5d0, 1851.1d0, 3.76d0, &  ! Es
    113.4d0, 1787.07d0, 3.89d0, &  ! Fm
    109.4d0, 1701.0d0, 3.93d0, &  ! Md
    105.4d0, 1578.18d0, 3.78d0 &  ! No
], [3, 102])

real(dp), parameter :: tssurf_vdw_params(3, 102) = reshape([ &
    4.5d0, 6.5d0, 3.1d0, &  ! H
    1.38d0, 1.46d0, 2.65d0, &  ! He
    164.2d0, 1387.0d0, 4.16d0, &  ! Li
    38.0d0, 214.0d0, 4.17d0, &  ! Be
