...
def parse_modules(lines):
    ...
    for line in lines:
        ...
        if word == 'module':
            module = (
                re.match(r'module\s+(\w+)\s*', line, re.IGNORECASE).group(1).lower()
            )
            if module != 'procedure':
...
def dep_tree(paths, macros=None):
    ...
    for path in paths:
        if path.suffix == '.F90':
            ...
            lines = (
                subprocess.run(args, capture_output=True).stdout.decode().split('\n')
            )
        else:
        ...
        for mod in modules:
            defs[mod] = path
    deps = {
        name: sorted({defs[mod] for mod in mods if mod not in {'iso_c_binding', 'mpi'}})
...
if __name__ == '__main__':
    ...
    configs = [
        ...
        ({'mbd_elsi.F90'}, ['WITH_SCALAPACK', 'WITH_MPI'], 'scalapack-mpi'),
    ]
    ...
    for filtered, macros, label in configs:
        ...
        (root / 'src' / f'{label}.make').write_text(
            template.substitute(
